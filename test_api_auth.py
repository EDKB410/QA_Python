...
class TestApiAuth:
    ...
    def test_api_auth(self):
        ...
        assert "auth_sid" in response1.cookies, "There is no auth cookie n the response"
        assert "x-csrf-token" in response1.headers, "There is no token header in the response"
        assert "user_id" in response1.json(), "There is no user_id in the response"

        auth_sid = response1.cookies.get("auth_sid")
        token = response1.headers.get("x-csrf-token")
        user_id_from_check_method = response1.json()["user_id"]
